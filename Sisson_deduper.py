	...
	with open(args.sam_file, 'r') as sam_file:						
		...
		for line in sam_file:
			if line[0] == '@': #if line is a header line
				...
				strand = int(bf.strand_reader(bitflag))
				corrected_position = int(bf.Position_corrector(position,cigar,strand))
	...
	with open(args.sam_file, 'r') as sam_file:
		...
		for line in sam_file:
			if line[0] == '@': #if line is a header line
				...
				line_split = line.split('\t')
				Qname = line_split[0]
				barcode = bf.barcode_puller(Qname)#pulls out the barcode from the Qname
				Chromosome = str(line_split[2])
				position = int(line_split[3])
				cigar =  str(line_split[5])
				bitflag = int(line_split[1])
				...
				try:  #Checks if barcode is in list
					ref_dict[barcode]
					try: #Checks if position has been seen before
						ref_dict[barcode][corrected_position]
						if strand in ref_dict[barcode][corrected_position]: #checks  if strand has been seen before
							pass
						else: #new strand, update and print line
							output_file.write(line)  
							ref_dict[barcode][corrected_position].append(strand)
					except:  #new position, update and print line
						output_file.write(line) 
						ref_dict[barcode][corrected_position] = []
						ref_dict[barcode][corrected_position].append(strand)
				except:  #new barcode, add it
					ref_dict[barcode] = {}
					ref_dict[barcode][corrected_position] = []
					ref_dict[barcode][corrected_position].append(strand)
	output_file.close()
